...
func (h *ImportHandler) ImportPlatesFromExcel(c *gin.Context) {
	...
	if err != nil {
		...
		// Очищаем список перед импортом
		deleted, err := h.accessListRepo.ClearPlates(list.ID)
		if err != nil {
			log.Printf("⚠️ Ошибка при очистке списка: %v", err)
		} else {
			log.Printf("🧹 Очищено %d номеров из списка '%s'", deleted, listName)
		}
	}
